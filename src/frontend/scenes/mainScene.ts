...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.matter.world.setBounds(0,0,1024,750, 50,true, true, true, true)
      ...
      this.socket.on("add opponent", (data: UserData)=>{
        let opponent = new Ball(this, data)     
        this.opponents.push(opponent)
         
      })
      this.socket.on("remove player", (pSocket)=>{
        let o:Ball[] = this.opponents.filter((player:Ball) => { return player.socketId == pSocket})
        if(o && o[0]){
          let p = o[0]
          this.opponents.splice(this.opponents.indexOf(p, 1))
          p.destroy()
      ...
      this.socket.on("update all", (data: any[])=>{
        data.forEach((p)=>{
          let o:Ball[] = this.opponents.filter((player:Ball) => { return player.socketId == p.socketId})       
          if(o && o[0] && o[0].socketId != this.player.socketId){
            let opponent = o[0]
            opponent.x = p.x
            opponent.y = p.y
